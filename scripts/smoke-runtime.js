...
const probes=[
  ...
  '(()=>{state=normalizeState({missionHistory:Array.from({length:12},(_,i)=>({date:"2026-09-"+String(i+1).padStart(2,"0")})),mastery:Object.fromEntries(["ma","mi","mo","mu","mé","la","li"].map(s=>[s,{attempts:4,correct:4,lastSeen:localDayKey()}]))});const pool=miniTextPool();return textComprehensionUnlocked()&&pool.length>=4&&!pool.some(x=>x.id==="domino-radio")})()',
